<template>
    <q-dialog v-model="localDialog">
      <q-card>
        ...
        <q-card-section>
          <q-form ref="form">
            <q-input v-model="localEditedItem.num" :rules="[rules.counter, rules.numIfNonEmpty]" label="№ Заявки" />
            <q-input v-model="formStgString" :rules="[rules.stgCheck, rules.max20Characters]" label="Продукт" @update:model-value="updateFormStg" />
            <div class="row">
  ...
  export default {
    ...
    setup(props, { emit }) {
      ...
      const rules = {
        ...
        numIfNonEmpty: value => /^[0-9]+$/.test(value) || 'Недопустимый номер заявки. Используйте только цифры',
        max20Characters: value => String(value).length <= 20 || 'Максимум 20 символов',
        stgCheck: value => /^[a-zA-Zа-яА-Я.'"0-9\s]+$/.test(value) || 'Используйте цифры (0-9) или буквы (a-z, A-Z, а-я, А-Я)',
        client_nameCheck: value => /^[a-zA-Zа-яА-Я\s.'"0-9]+$/.test(value) || 'Используйте цифры (0-9) или буквы (a-z, A-Z, а-я, А-Я). Максимум 30 символов',
        personPhoneCheck: value => /^[0-9]{11}$/.test(value) || 'Недопустимый номер телефона. Должен содержать ровно 11 цифр'
      };
